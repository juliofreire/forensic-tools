...
# Counting occurency of hours
contagem_ocorrencias = df.groupby(["Data", "Hora"]).size().unstack(fill_value=0)

# Plotting occurency of hours in one day 
for dia in contagem_ocorrencias.index:
    plt.figure(figsize=(12, 6))
    plt.plot(contagem_ocorrencias.columns, contagem_ocorrencias.loc[dia], marker='o', label=str(dia))
...
# Counting occurency of days
contagem_ocorrencias = df.groupby(["Mes", "Dia"]).size().unstack(fill_value=0)

# Plotting occurency of day in one month
for mes in contagem_ocorrencias.index:
    plt.figure(figsize=(12, 6))
    plt.plot(contagem_ocorrencias.columns, contagem_ocorrencias.loc[mes], marker='o', label=f'Mês {mes}')
